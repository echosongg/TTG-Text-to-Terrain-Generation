...
//根据给定的分布表 table 填充高度值数组 m_fPosition，将高度值简化并分布在 -1.0 到 1.0 的范围内。
void CDesignerWorld::SetValueTable(int table[], const int n){
  ...
  if(sum != SIZE){
    ...
    return; //fail and bail 这里的table是distribution table
  } //if
  ...
  for(int j=0; j<table[n-1]; j++)
    m_fPosition[k++] = 1.0f;  //m_fPositon代表256个高度的不同可能
} //SetValueTable
...
float CDesignerWorld::noise(float x, float z){
  ...
  return lerp(sz, v0, v1);//返回一个插值，这个是论文的核心部分，用于生成噪声
} //noise